	  ...
	  <div class="bord_list">
		<table class="bord_table" summary="이표는 번호,제목,글쓴이,날자,조회수로 구성되어 있습니다">
			...
			<c:forEach var="down" items="${list}">
				<tr>
					<td>${down.bno}</td>
					<td class="title"><a href="pdsview.do?bno=${down.bno }">${down.title}</a></td>
					<td>
					<c:if test="${down.filerealname != null}">
						<a href="${pageContext.request.contextPath}/download.do?file=${down.filerealname}"><img src="images/file.png"></a>
					</c:if>
					</td>
					<td>${down.writer}</td>
					<td>${down.wdate}</td>
		...
		<div class="paging">
			...
			<a href="pdswrite.do" class="btn_write">글쓰기</a>
		</div>
	  </div>
	</div>
	<!-- end contents -->
