	  ...
	  <div class="bord_list">
		<table class="bord_table" summary="이표는 번호,제목,답변상태, 작성자, 작성일, 조회수로 구성되어 있습니다">
			...
			<c:forEach var="list" items="${list}">
				<tr>
					<td>${list.bno}</td>
					<td class="title"><a href="/qna/qaview.do?bno=${list.bno}">${list.title}</a></td>
					...
					<td>${list.writer}</td>
					<td><fmt:formatDate var="regdate" value="${list.regdate}" pattern="yyyy-MM-dd"/>${regdate}</td>
		...
		<div class="paging">
			...
			<sec:authorize access="isAuthenticated()">
				<a href="/qna/qawrite.do" class="btn_write" id="btn_qawrite">글쓰기</a>
			</sec:authorize>
		</div>
	  </div>
	</div>
